...
<div 
    x-data="{
        ...
        init() {
            ...
            if (this.autoSlide) {
                setInterval(() => {
                    if (this.autoSlide && this.isScrollable) this.autoNext();
                }, this.interval);
            }
        },

        // Dynamically determine how many cards should show per screen size
        setResponsivePerPage() {
            const width = window.innerWidth;
            if (width < 640) {
                this.perPage = 1;
            } else if (width < 1024) {
                this.perPage = this.cardmd; // Default to cardlg if larger than 640px but smaller than 1024px
            } else if (width < 1280) {
                this.perPage = this.cardlg; // Default to cardxl if larger than 1024px but smaller than 1280px
            } else {
                this.perPage = this.cardxl; // Default to cardxl if larger than 1280px
            }
        },
...
>    
    ...
    <template x-if="showNav && isScrollable">
        <button 
            @click="prev" 
            class="absolute h-10 w-10 left-2 top-1/2 transform -translate-y-1/2 -translate-x-1/5 bg-bacancy-primary/80 hover:bg-bacancy-primary/90 text-white p-2 rounded-full shadow z-20"
        >
            &#8592;
        </button>
    </template>
    ...
    <div 
        x-ref="cards" 
        class="flex gap-4 overflow-x-auto scroll-smooth no-scrollbar snap-x snap-mandatory"
    >
        {{ $slot }}
    </div>
