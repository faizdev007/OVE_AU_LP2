...
<div class="bg-[#EAEAEA]" section="hero-section">
    ...
    @else
        <div class="relative py-2 pb-6 flex-1 overflow-hidden max-w-7xl mx-auto dark:border-neutral-700">
            <div class="md:flex items-center justify-center">
                <div class="flex-1 grid xl:gap-6 md:py-0 py-3 gap-2 px-6">
                    <h1 class="text-2xl xl:text-[2.6rem] font-extrabold text-gray-900 ">{{$hero_title_one}} <span class="text-bacancy-primary">{{$hero_title_two}}</span> {{$hero_title_three}}</h1>
                    <p class="mt-2 text-black xl:text-xl md:text-sm  font-bold mb-3">{{$hero_subtitle}}</p>
                    <div class="">
                        <ul class="flex flex-col justify-center gap-2 xl:text-xl md:text-xs text-sm list-disc list-inside">
                            @php
                                $list = $hero_lists;
                            @endphp
                            @foreach($list as $item)
                                <li class="text-gray-900  flex gap-2">
                                    <span class="text-bacancy-primary">
                                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" class="md:size-9 size-6">
                    ...
                    <flux:modal.trigger name="book-a-call">
                        <x-action-button x-data="" x-on:click.prevent="$dispatch('open-modal', 'book-a-call')" title="{{ $btntext }}" class="!text-white w-max !bg-bacancy-primary hover:!bg-[#1D4ED8] focus:!ring-[#1D4ED8]" />
                    </flux:modal.trigger>
                </div>
                <div class="flex-1 relative md:me-[3%] md:w-[300px] md:m-[20px] my-[20px] overflow-hidden flex items-center justify-center">
                    <div class="absolute rounded-full w-[75%] h-[90%] xl:mt-11 mt-10 bg-bacancy-lightblue z-10"></div>
                    <div class="absolute rounded-full w-[60%] h-[75%] xl:mt-11 mt-10 justify-center items-center bg-bacancy-lightblue z-10"></div>
                    <img loading="eager" fetchpriority="high" decoding="async" src="{{ asset('assets/bacancy/hero_section_person.webp') }}" alt="{{ config('app.name') }}" width="80%" height="80%" class="w-[80%] h-[80%%] relative z-20" />
                    <div class="absolute rounded-full w-full h-full z-30 p-2">
                        <div class="relative w-full h-full">
                            ...
                            <div class="bg-white absolute w-[30%] top-[25%] xl:text-3xl text-xl font-bold text-center end-0 text-black p-2 border border-bacancy-primary rounded-md">{{$box2}}
