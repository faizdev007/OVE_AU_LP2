<div class="py-12" section="companies-logo">
                        ...
                        <div class="flex flex-wrap gap-3">
                            <template x-for="(logo, index) in previews" :key="logo">
                                <div class="relative w-24 border rounded">
                                    <img :src="'{{ asset('') }}' + logo" alt="" class="w-full h-full object-cover rounded">
                                    <button type="button"
    ...
    <div class="overflow-hidden bg-white">
        <div class="logo-slider flex w-max md:space-x-8 space-x-4">
            @foreach ($companyLogos as $index => $logo)
                <img src="{{ asset($logo) }}" class="h-12 shadow w-auto border rounded" alt="Logo {{ $index + 1 }}">
            @endforeach
            @foreach ($companyLogos as $index => $logo)
                <img src="{{ asset($logo) }}" class="h-12 shadow w-auto border rounded" alt="Logo {{ $index + 1 }}">
            @endforeach
